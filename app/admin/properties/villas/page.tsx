...
export default async function VillasPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  const params = await searchParams;
  await connectDB();

  const query: any = { type: "villa" };
  if (params.status && params.status !== "all") {
    if (params.status === "for-sale" || params.status === "for-rent") query.condition = params.status;
    else query.status = params.status;
  }

  const [villas, counts] = await Promise.all([
    Listing.find(query).sort({ createdAt: -1 }).lean(),
    Promise.all(TABS.map((s) => {
      if (s === "all") return Listing.countDocuments({ type: "villa" });
      if (s === "for-sale" || s === "for-rent") return Listing.countDocuments({ type: "villa", condition: s });
      return Listing.countDocuments({ type: "villa", status: s });
    })),
  ]);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Admin / Properties / Villas</p>
        </div>
        <Link
        ...
        >
          <Plus className="w-4 h-4" /> Add Villa
        </Link>
      </div>
      ...
      <PropertyTable properties={JSON.parse(JSON.stringify(villas))} propertyType="villa" />
    </div>
  );
}
