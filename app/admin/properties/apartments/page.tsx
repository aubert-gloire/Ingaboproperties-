...
export default async function ApartmentsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  const params = await searchParams;
  await connectDB();

  const query: any = { type: "apartment" };
  ...
  const [apartments, counts] = await Promise.all([
    ...
    Promise.all(TABS.map((s) => {
      if (s === "all") return Listing.countDocuments({ type: "apartment" });
      if (s === "for-sale" || s === "for-rent") return Listing.countDocuments({ type: "apartment", condition: s });
      return Listing.countDocuments({ type: "apartment", status: s });
    })),
  ]);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Admin / Properties / Apartments</p>
        </div>
        <Link
      ...
      <PropertyTable properties={JSON.parse(JSON.stringify(apartments))} propertyType="apartment" />
    </div>
  );
}
