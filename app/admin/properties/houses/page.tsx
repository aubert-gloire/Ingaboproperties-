...
export default async function HousesPage({ searchParams }: { searchParams: Promise<{ status?: string; search?: string }> }) {
  const params = await searchParams;
  await connectDB();

  const query: any = { type: "house" };
  ...
  const [houses, counts] = await Promise.all([
    Listing.find(query).sort({ createdAt: -1 }).lean(),
    Promise.all(TABS.map((s) => {
      if (s === "all") return Listing.countDocuments({ type: "house" });
      if (s === "for-sale" || s === "for-rent") return Listing.countDocuments({ type: "house", condition: s });
      return Listing.countDocuments({ type: "house", status: s });
    })),
  ]);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Houses</h1>
          <p className="text-gray-500 text-sm">Admin / Properties / Houses</p>
        </div>
        <Link
          href="/admin/properties/new?type=house"
        ...
        {TABS.map((s, i) => (
          <Link key={s} href={`?status=${s}`}
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
              (params.status || "all") === s ? "bg-forest-800 text-white" : "text-gray-600 hover:bg-gray-50"
            }`}>
      ...
      <PropertyTable properties={JSON.parse(JSON.stringify(houses))} propertyType="house" />
    </div>
  );
}
